...
class TestExamplePoolDiscovery:
    """Example: find all v4 pools for a token pair using V4pools."""

    def test_find_eth_usdc_pools(self, pool_service: V4pools) -> None:
        """
        V4pools.fetch_poolkey_data scans PoolManager Initialize events to
        build a local index of all pools. get_poolkeys_sublist then filters
        by token pair.
        """
        # Fetch a small slice of history to keep the test fast

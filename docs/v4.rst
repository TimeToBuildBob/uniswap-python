...
amount. The ``qtycap`` parameter sets the **minimum** acceptable output; the
transaction reverts if slippage exceeds this threshold.
...
:func:`~uniswap.Uniswap4.make_swap_output` buys an exact output amount. Here
``qtycap`` is the **maximum** input you are willing to spend:

.. code:: python

    # Buy exactly 100 USDC; pay at most 0.05 ETH
    expected_eth = uni.get_price_output(ETH_ADDRESS, USDC, 100 * 10**6, fee=500, tick_spacing=10)
    max_eth = int(expected_eth * 1.01)  # 1% slippage tolerance
...
.. code:: python
    ...
    value = uni.get_position_value(token_id)
...
.. code:: python
    ...
    uni.collect_fees(token_id=token_id)

    # Remove half the liquidity
    liquidity = uni.get_position_info(token_id)["liquidity"]
    uni.decrease_liquidity(token_id=token_id, liquidity=liquidity // 2)

    # Burn the position NFT once fully emptied
    uni.burn_position(token_id=token_id)
...
.. code:: python
    ...
    slot0 = uni.stateview_get_slot0(ETH_ADDRESS, USDC, fee=500, tick_spacing=10)
    print(f"sqrtPriceX96: {slot0['sqrtPriceX96']}")
    print(f"current tick: {slot0['tick']}")

    # Total in-range liquidity
    liquidity = uni.stateview_get_liquidity(ETH_ADDRESS, USDC, fee=500, tick_spacing=10)
